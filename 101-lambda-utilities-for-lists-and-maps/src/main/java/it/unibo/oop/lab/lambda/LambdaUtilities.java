...
public final class LambdaUtilities {
    ...
    public static <T> List<Optional<T>> optFilter(final List<T> list, final Predicate<T> pre) {
         ...
         */
        final List<Optional<T>> items = new ArrayList<>();
        list.forEach(item -> {
            final Optional<T> filteredOptional = Optional.of(item).filter(pre);
            items.add(filteredOptional);
        });
        return items;
    }
    ...
    public static <R, T> Map<R, Set<T>> group(final List<T> list, final Function<T, R> op) {
        ...
        list.forEach(item -> {
            final R key = op.apply(item);
            map.merge(key, new HashSet<>(Arrays.asList(item)), (t1, t2) -> {
                t1.addAll(t2);
                return t1;
            });
        });
    ...
    public static <K, V> Map<K, V> fill(final Map<K, Optional<V>> map, final Supplier<V> def) {
        ...
        map.forEach((key, value) -> {
            outMap.put(key, value.orElse(def.get()));
        });
        return outMap;
    }
